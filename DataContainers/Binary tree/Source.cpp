...
private:
	...
	void insert(const int& Data, Element* Root)  //Element* Root - указатель на ветку (поддерево)
	{
		...
		else
		{
			...
			//{
			//	Root->pRight = new Element(Data); // Создаём его 
			//}
			//else // В противном случае
			//{
			//	insert(Data, Root->pRight); // Идём дальше
			//}
